...
void UHeroEquipBoardWidget::InitHeroEquipBoard(UInventoryWidget* widget_ptr, int32 hero_idx)
{
	hero_idx_ = hero_idx;
...
void UHeroEquipBoardWidget::LoadHeroData()
{
	...
	{
		FSpawnData data_cache = transition_system->GetSpawnData(hero_idx_);
...
void UHeroEquipBoardWidget::UpdateHeroData()
{
	...
	if(transition_system->GetSpawnData().IsEmpty() == false)
	{
		...
		transition_system->UpdateSpawnDataIdx(hero_idx_, data_cache);
	}
}
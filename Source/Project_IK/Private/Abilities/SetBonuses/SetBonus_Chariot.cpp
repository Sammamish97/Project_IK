...
void USetBonus_Chariot::ActivateEdgeBonus()
{
	...
	hero_cache_->ApplyBuff(FBuff(TEXT("Chariot_Edge"), ECharacterStatType::AttackSpeed, 20, false, 100.f));
}
...
void USetBonus_Chariot::ActivateTriangleBonus()
{
	...
	auto& delegate_array = hero_cache_->hero_dmg_event_map_.FindOrAdd(EHeroEvent::OnReload);
	delegate_array.AddDefaulted();
	delegate_array.Last().BindUObject(this, &USetBonus_Chariot::TriangleBonus);
}
...
void USetBonus_Chariot::ActivateHexagonBonus()
{
	...
	auto& delegate_array = hero_cache_->hero_dmg_event_map_.FindOrAdd(EHeroEvent::OnReload);
	delegate_array.AddDefaulted();
	delegate_array.Last().BindUObject(this, &USetBonus_Chariot::HexagonBonus);
}

FDamageData USetBonus_Chariot::TriangleBonus(FDamageData dmg_data)
{
	hero_cache_->ApplyBuff(FBuff(TEXT("Chariot_Triangle"), ECharacterStatType::AttackSpeed, 50, false, 2.f));
	return dmg_data;
}

FDamageData USetBonus_Chariot::HexagonBonus(FDamageData dmg_data)
{
	hero_cache_->ApplyBuff(FBuff(TEXT("Chariot_Triangle"), ECharacterStatType::CriticalHitRate, 50, false, 2.f));
	return dmg_data;
}
